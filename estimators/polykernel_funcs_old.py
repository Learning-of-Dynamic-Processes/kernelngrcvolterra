...
class PolynomialKernel:
    ...
    def Train(self, training_input, training_teacher):
        ...
        Kernel2 = poly_gram_train_njit(self.ninputs, self.ndim, self.ndelays, self.Delays, self.deg)
        print(f"With njit: {time.time() - start}")
        print(np.allclose(self.Kernel, Kernel2))
    ...
    def Forecast(self, testing_input):
        ...
        # Initialise store for the forecast output
        output2 = np.zeros((self.nhorizon, self.ntargets))
        ...
        for t in range(self.nhorizon):
            ...
            # Compute forecast using new Gram input column
            for target in range(self.ntargets):
                output2[t, target] = np.dot(self.alpha[:, target], Kernel_new_col[self.washout:self.ninputs-self.ndelays+1]) + self.alpha0[target]
    ...
    def PathContinue(self, latest_input, nhorizon):  
        ...
        """
        latest_input_for_later = latest_input
        # Original
        start = time.time()
        # Assign testing input instance attributes
        ...
        for t in range(self.nhorizon):
            ...
            Kernel_new_col2 = poly_gram_pathcontinue_njit(t, self.ndelays, self.ndim, self.ninputs, Delay_new, self.Delays, self.deg)
            
            if np.allclose(Kernel_new_col, Kernel_new_col2) is False: 
                print(Kernel_new_col)
                print(Kernel_new_col2)
            ...
            for target in range(self.ntargets):
                output2[t, target] = np.dot(self.alpha[:, target], Kernel_new_col2[self.washout:self.ninputs-self.ndelays+1]) + self.alpha0[target]
            
            if np.allclose(output[t], output2[t]):
                print("time ", t)
                print(Kernel_new_col[self.washout:self.ninputs-self.ndelays+1][0])
                print(Kernel_new_col2[self.washout:self.ninputs-self.ndelays+1][0])
            # Reassign last delay column
            Delay_last_col = Delay_new[:, t]
        ...
        # New
        start = time.time()
        latest_input = latest_input_for_later
        # Assign testing input instance attributes
        self.nhorizon = nhorizon
        ...
        for t in range(self.nhorizon):
            ...
            # Reassign last delay column
            Delay_last_col = Delay_new[:, t]
            latest_input = output2[t, :]
        
        print(f"With njit: {time.time() - start}")
        print(np.allclose(output, output2))
        plot_data([output - output2])
