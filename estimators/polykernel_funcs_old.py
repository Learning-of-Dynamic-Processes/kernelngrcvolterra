...
### File for checks. To delete after commit.
        ...
        for i in range(self.ninputs-self.ndelays+1):
            for j in range(i+1):
                ...
                if (i == 300 and j == 300) or (i==500 and j==200) or (i==500 and j ==500): 
                    #print(self.Delays[:, i][0])
                    #print(self.Delays[:, j][0])
                    print(i, j)
                    dot_prod = 0
                    for val_id in range(len(self.Delays[:, i])):
                        dot_prod = dot_prod + self.Delays[val_id, i] * self.Delays[val_id, j]
                    print(dot_prod)
                    print(np.dot(self.Delays[:, i], self.Delays[:, j]))
                    #print((1 + np.dot(self.Delays[:, i], self.Delays[:, j])))
                    #print((1 + np.dot(self.Delays[:, i], self.Delays[:, j]))**self.deg)
